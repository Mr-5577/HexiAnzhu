...
import { ContractDetailColumns } from "./project-columns";
...
defineOptions({ name: "contract-detail" });
...
const filteredData = ref<any[]>([]); // 过滤后的数据列表
...
const getTableList = async () => {
  try {
    ...
    const res = await assetManagementApi.getSaleAsstListSign(params);
    if (res.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const fileBlob = await assetManagementApi.exportSaleAsstListSign(params);
