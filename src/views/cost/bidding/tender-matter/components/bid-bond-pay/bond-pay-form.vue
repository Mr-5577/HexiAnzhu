  ...
  <div class="basic-form-content">
    ...
    <div class="form-scroll-area">
      ...
      >
        ...
        <!-- 投标保证金缴纳列表 -->
        <div class="item-card">
          <div class="section-title">投标保证金缴纳列表</div>
            ...
            >
              <template #actionBar>
                <div class="actionBar-buttons">
                  <el-button type="primary" size="small" @click="handleAddPay">
                    新增
                  </el-button>
                </div>
              </template>
              ...
              <template #actions="{ row }">
                <div class="actions-btn">
                  ...
                  <el-button link type="danger" @click="handleDeletePay(row)">
                    删除
                  </el-button>
                </div>
              </template>
...
const tenderItemOptions = ref([]); // 招标明细事项列表
...
// 数据字典
const { getDictList, loadDicts } = useDict([dictMapping.payMethod], {
  treeDictCodes: [],
});
const payMethodOptions = ref([]); // 支付方式
...
const dynamicColumns = computed<EditableColumn[]>(() => [
  { type: "index", label: "序号", width: 60 },
  {
    ...
    optionValueField: "id",
    options: tenderItemOptions.value || [],
  },
  {
    // 楼栋范围由招标明细事项带出回填，不可编辑。新增的字段
    prop: "bldNames",
    label: "楼栋范围",
    minWidth: 150,
    editable: false,
  },
  {
    ...
    width: 150,
    editable: true,
    clickable: true, // 可点击单元格，选择供应商
    showOverflowTooltip: false,
  },
  {
    // 应交保证金金额由招标明细事项带出回填,不可编辑
    prop: "needAmount",
    label: "应交保证金金额",
    minWidth: 150,
    editable: false,
...
const detailColumns = [
  ...
  { prop: "bldNames", label: "楼栋范围" },
  { prop: "supName", label: "供应商名称" },
  {
    prop: "needAmount",
    label: "应交保证金金额",
    width: 150,
    formatter: (row) => formatNumberDisplay(row.needAmount),
  },
  {
    prop: "recvAmount",
    label: "实交保证金金额",
    width: 150,
    formatter: (row) => formatNumberDisplay(row.recvAmount),
  },
  {
    prop: "recvMethodName",
    label: "缴纳方式",
    ...
    formatter: (row) => {
      const selectedOption = payMethodOptions.value.find(
        (option: any) => option.id === row.recvMethod,
      );
      return selectedOption ? selectedOption.dicLabel : "";
    },
  },
  { prop: "recvDate", label: "缴纳日期", width: 150 },
  { slot: "recvAnnexName", label: "缴纳凭证附件" },
];
...
const handleTableSave = async (data) => {
  ...
  if (column === "tenderItemName") {
    const selectedOption = tenderItemOptions.value.find(
      (option: any) => option.id === newValue,
    );
    updateRow(rowIndex, {
      tenderId: selectedOption.tenderId,
      tenderItemId: newValue,
      tenderItemName: selectedOption.tenderItemName,
      bldIds: selectedOption.bldIds,
      bldNames: selectedOption.bldNames,
      needAmount: selectedOption.bidBondAmount,
    });
    return;
  }
  // 缴纳方式变更时，同步更新缴纳方式名称
...
const handleViewAnnex = async (row: any) => {
  ...
  try {
    const res = await commonApi.getFileList({ annexId: row.recvAnnexId });
...
const handleAddPay = () => {
  const newRowData = {
    ...
    supName: "", // 供应商名称
    needAmount: 0, // 应交保证金金额
    recvAmount: 0, // 实交保证金金额
    recvMethod: undefined, // 缴纳方式
    recvMethodName: "", // 缴纳方式名称
    recvDate: "", // 缴纳日期
    recvAnnexId: undefined, // 缴纳凭证附件ID
    recvAnnexName: "", // 缴纳凭证附件名称
  };
  tableData.value = [...tableData.value, newRowData];
};

const handleDeletePay = (row: any) => {
...
const getTreeProjectOptions = async () => {
  try {
    const res = await projectAreaApi.getMguProjList();
    if (res.code === 200) {
      treeProjectOptions.value = res.data || [];
    }
  } catch (error) {
    console.error("获取项目列表失败:", error);
  }
...
const initAddTableData = async () => {
  ...
  if (items && items.length > 0) {
    ...
    //   supId: undefined,
    //   supName: "",
    //   needAmount: item.bidBondAmount,
    //   recvAmount: 0,
    //   recvMethod: undefined,
    //   recvMethodName: "",
    //   recvDate: "",
    //   recvAnnexId: undefined,
    //   recvAnnexName: "",
    // }));
    // tableData.value = initialTableList;

    // 保存事项明细列表，提供给招标保证金缴纳列表选择
    const newData = items.map((item: any) => ({
      ...item,
      uuid: uuidv4(),
    }));
    tenderItemOptions.value = newData || [];
  } else {
    tableData.value = [];
  }
};
...
const getBillDetail = async () => {
  ...
  try {
    ...
    if (res.code === 200 && res.data) {
      ...
      await initAddTableData(); // 通过详情获取事项明细列表
      ...
      if (bondRecvs && bondRecvs.length > 0) {
        const initTableList = bondRecvs.map((item) => ({
          ...item,
          uuid: uuidv4(),
          tenderId: tenderId,
        }));
...
const goBack = () => {
  if (isAdd.value) {
    tagsStore.closeTagByPath("/bidding/bid-bond-pay/add");
  }
  if (isEdit.value) {
    tagsStore.closeTagByPath("/bidding/bid-bond-pay/edit");
  }
  router.go(-1); // 返回上个页面
};
...
const validateForm = () => {
  ...
  if (tableData.value.some((item) => !item.recvMethod)) {
    ElMessage.error("请选择列表中的缴纳方式");
    return false;
  }
  if (tableData.value.some((item) => !item.recvDate)) {
    ElMessage.error("请选择列表中的缴纳日期");
    return false;
  }
  return true;
};
