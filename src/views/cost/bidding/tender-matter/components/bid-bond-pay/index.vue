    ...
    >
      ...
      <template #expand="{ row }">
        <div class="expand-table-wrapper">
          ...
          <div v-if="!row.bondRecvs?.length" class="empty-detail">
            暂无明细数据
          </div>
        </div>
      </template>
...
import { BidBondRecvBill } from "@/types/cost/bidding-management-type";

defineOptions({ name: "bid-bond-pay" });
...
const detailColumns: TableColumnItem[] = [
  ...
  { prop: "recvAnnexId", label: "缴纳凭证" },
];

const tableData = ref<BidBondRecvBill[]>([]);
...
const getTableList = async () => {
  try {
    ...
    const params = {
      ...
      bizItemCode: "ZB_BZJ", // 投标保证金缴纳
    };
    const res = await biddingManageApi.getBillList(params);
    if (res.code === 200 && res.data) {
      tableData.value = res.data.map((item: BidBondRecvBill) => ({
        ...item.bill,
        bondRecvs: item.bondRecvs || [],
      }));
    }
  } catch (error) {
    console.error("获取投标保证金缴纳列表失败:", error);
  } finally {
