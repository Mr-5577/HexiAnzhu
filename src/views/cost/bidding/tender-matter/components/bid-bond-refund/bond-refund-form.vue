<!-- 投标保证金退还 -- 新增/编辑/详情 -->
<template>
  <div class="bond-refund-form-page">
    <!-- 基本信息 -->
    <basic-info
      :data="detailData"
    ...
    <div class="form-section">
      <h3 class="section-title">投标保证金退还列表</h3>
...
const handleBatchSave = async () => {
  ...
  if (tableData.value.some((item) => !item.refundType)) {
    ElMessage.error("请填写列表中的退还方式");
    return;
  }
  if (tableData.value.some((item) => !item.refundAmount)) {
    ElMessage.error("请填写列表中的退还金额");
    return;
  }
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      if (targetData) {
        ...
        tableData.value = targetData.bondRefunds || []; // 初始化表格数据
      }
    }
  } catch (error) {
...
const initDictData = async () => {
  ...
  refundMethodOptions.value = getDictList(dictMapping.refundMethod); // 退还方式
};
