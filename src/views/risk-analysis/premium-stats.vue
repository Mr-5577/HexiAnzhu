    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      ...
      <el-form-item>
        ...
        <el-button
          ...
          @click="handleExport"
          :disabled="!menuStore.hasExactPermission('premium-stats:export')"
        >
          导出
        </el-button>
      </el-form-item>
...
import { premiumStatsColumns } from "./project-columns";
import { useSalesData } from "@/composables/use-sales";
import { dateUtil } from "@/utils/date-util";
import { assetManagementApi } from "@/api/sales/asset-management-api";
import { PremiumStatsInterface } from "@/types/sales/risk-analysis-type";
...
defineOptions({ name: "premium-stats" });
...
const getTableList = async () => {
  try {
    ...
    const res = await assetManagementApi.getPremiumCountProj(params);
    if (res.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const fileBlob = await assetManagementApi.exportPremiumCountProj(params);
