<!-- 认购转签约明细 -->
<template>
  <div class="conversion-detail-page">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      ...
      <el-form-item>
        ...
        <el-button
          ...
          @click="handleExport"
          :disabled="!menuStore.hasExactPermission('conversion-detail:export')"
        >
          导出
        </el-button>
      </el-form-item>
...
import { conversionDetailColumns } from "./project-columns";
import { useSalesData } from "@/composables/use-sales";
import { dateUtil } from "@/utils/date-util";
import { assetManagementApi } from "@/api/asset-management-api";
import { ConversionDetailInterface } from "@/types/risk-analysis-type";
...
defineOptions({ name: "conversion-detail" });
...
const tableData = ref<ConversionDetailInterface[]>([]);
const allTableList = ref<ConversionDetailInterface[]>([]);
...
const getParams = () => {
  ...
  return {
    ...
    isShowTel: menuStore.hasExactPermission("conversion-detail:showTel"),
  };
};
// 获取列表
const getTableList = async () => {
  try {
    ...
    const res = await assetManagementApi.getOrderToSignPeriodRoom(params);
    if (res.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const fileBlob = await assetManagementApi.exportOrderToSignPeriodRoom(
      params
    );
