...
<template>
  <div class="premium-detail-page">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      ...
      <el-form-item label="认购时间" prop="time">
        <el-date-picker
      ...
      <el-form-item>
        ...
        <el-button
          ...
          @click="handleExport"
          :disabled="!menuStore.hasExactPermission('premium-detail:export')"
        >
          导出
        </el-button>
      </el-form-item>
...
import { premiumDetailColumns } from "./project-columns";
import { useSalesData } from "@/composables/use-sales";
import { dateUtil } from "@/utils/date-util";
import { assetManagementApi } from "@/api/sales/asset-management-api";
import { PremiumDetailInterface } from "@/types/sales/risk-analysis-type";
...
defineOptions({ name: "premium-detail" });
...
const tableData = ref<PremiumDetailInterface[]>([]);
const allTableList = ref<PremiumDetailInterface[]>([]);
...
const getParams = () => {
  ...
  return {
    ...
    isShowTel: menuStore.hasExactPermission("premium-detail:showTel"),
  };
};
// 获取列表
const getTableList = async () => {
  try {
    ...
    const res = await assetManagementApi.getPremiumCountRoom(params);
    if (res.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const fileBlob = await assetManagementApi.exportPremiumCountRoom(params);
