...
import { pendingStatsColumns } from "./project-columns";
import { useSalesData } from "@/composables/use-sales";
import { dateUtil } from "@/utils/date-util";
import { assetManagementApi } from "@/api/asset-management-api";
import { PendingStatsInterface } from "@/types/risk-analysis-type";
...
defineOptions({ name: "pending-stats" });
...
const getTableList = async () => {
  try {
    ...
    const res = await assetManagementApi.getOrderNotSignInfoProj(params);
    if (res.code === 200) {
...
const handleExport = async () => {
  try {
    ...
    const fileBlob = await assetManagementApi.exportOrderNotSignInfoProj(
      params
    );
